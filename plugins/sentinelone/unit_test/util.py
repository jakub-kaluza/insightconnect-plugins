...
class Util:
    ...
    def mocked_requests_get(*args, **kwargs):
        ...
        if args[0] == "https://rapid7.com/web/api/v2.1/users/login":
            ...
            if "params" in kwargs:
                threat_ids = kwargs["params"].get("ids")
                if "valid_threat_id_1" in threat_ids:
                    return MockResponse("get_valid_threat_1", 200)
                elif "valid_threat_id_2" in threat_ids:
                    return MockResponse("get_valid_threat_2", 200)
                elif "same_status_threat_id_1" in threat_ids:
                    return MockResponse("get_same_status_threat_1", 200)
                elif "non_existing_threat_id_1" in threat_ids:
                    return MockResponse("get_non_existing_threat_1", 200)
        elif args[1] == "https://rapid7.com/web/api/v2.1/cloud-detection/alerts":
            if "params" in kwargs:
                alert_ids = kwargs["params"].get("ids")
                if "valid_alert_id_1" in alert_ids:
                    return MockResponse("get_valid_alert_1", 200)
                elif "valid_alert_id_2" in alert_ids:
                    return MockResponse("get_valid_alert_2", 200)
                elif "same_status_alert_id_1" in alert_ids:
                    return MockResponse("get_same_status_alert_1", 200)
                elif "non_existing_alert_id_1" in alert_ids:
                    return MockResponse("get_non_existing_alert_1", 200)
        elif args[1] == "https://rapid7.com/web/api/v2.1/threats/incident":
            return MockResponse(f"affected_{len(kwargs.get('json', {}).get('filter', {}).get('ids'))}", 200)
        elif args[1] == "https://rapid7.com/web/api/v2.1/cloud-detection/alerts/incident":
            return MockResponse(f"affected_{len(kwargs.get('json', {}).get('filter', {}).get('ids'))}", 200)
        elif args[1] == "https://rapid7.com/web/api/v2.1/threats/analyst-verdict":
            return MockResponse(f"affected_{len(kwargs.get('json', {}).get('filter', {}).get('ids'))}", 200)
        elif args[1] == "https://rapid7.com/web/api/v2.1/cloud-detection/alerts/analyst-verdict":
            return MockResponse(f"affected_{len(kwargs.get('json', {}).get('filter', {}).get('ids'))}", 200)
